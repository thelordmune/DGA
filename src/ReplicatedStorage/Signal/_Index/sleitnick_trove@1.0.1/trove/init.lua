...
function Trove:Clone<T>(instance: T): T
	if self._cleaning then
...
function Trove:Add<T>(object: T, cleanupMethod: string?): T
	...
	local cleanup = GetObjectCleanupFunction(object :: any, cleanupMethod)
...
function Trove:_cleanupObject(object, cleanupMethod)
	...
	elseif cleanupMethod == THREAD_MARKER then
		pcall(task.cancel, object :: any)
	else
		(object :: any)[cleanupMethod](object)
	end
end
...
type TroveClassInternal = {
	_findAndRemoveFromObjects: (self: TroveClass, object: Object, cleanup: boolean) -> boolean,
	_cleanupObject: (self: TroveClass, object: Object, cleanupMethod: string?) -> (),
}
		
export type TroveClass = {
	Extend: (self: TroveClass) -> TroveClass,
	
	Construct: <T>(self: TroveClass, class: T, ...any) -> T,
	Connect: (self: TroveClass, signal: Signal, (...any) -> (...any)) -> Connection,
	BindToRenderStep: (name: string, priority: number, callback: (deltaTime: number) -> ()) -> (),
	Add: <T>(self: TroveClass, object: T, cleanupMethod: string?) -> T,
	AddPromise: <T>(self: TroveClass, promise: T) -> T,
	Remove: (self: TroveClass, object: Object) -> boolean,
	Clone: <T>(self: TroveClass, object: T) -> T,
	
	AttachToInstance: (self: TroveClass, instance: Instance) -> RBXScriptConnection,
	
	Clean: (self: TroveClass) -> (),
	Destroy: (self: TroveClass) -> (),
}

export type Trove = TroveClass & {
	_cleaning: boolean,
	_objects: { any }
}
