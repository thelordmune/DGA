...
-- Constants
local SERVER_MAX_REQUIRE_TIME = 5
local CLIENT_MAX_REQUIRE_TIME = 1
...
function Loader.LoadDescendants(parent: Instance, predicate: PredicateFn?): { [string]: any }
	...
	for _, descendant in parent:GetDescendants() do
		if descendant:IsA("ModuleScript") then
			...
			local MAX_REQUIRE_TIME = if RunService:IsServer() then SERVER_MAX_REQUIRE_TIME else CLIENT_MAX_REQUIRE_TIME
			local thread = task.delay(MAX_REQUIRE_TIME, function()
				task.spawn(error, `"{descendant.Name}" module took more than {MAX_REQUIRE_TIME}s to be required!`)
			end)
