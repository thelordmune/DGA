	...
	local actions: {ActionData} = {
		{
			...
			action = function(): string?
				...
				return `{hasSpecialAnimationType and `{mainConfig.Setting.SpecialAnimationType}{weaponType}Run` or `{weaponType}Run`}`
		...
		},
		{
			...
			action = function(): string?
				return `{hasSpecialAnimationType and `{mainConfig.Setting.SpecialAnimationType}Run` or "Run"}` 
			end
		}
	}
