...
function Debris.Normal(AnchorPoint: CFrame, Settings: Rising, State: {})
	...
	-- Check if we have a surface normal to work with
	...
	if surfaceNormal then
		...
		surfaceAlignedCFrame = CFrame.fromMatrix(AnchorPoint.Position, rightVector, upVector, forwardVector)
	end
	...
	for _ = 1, Settings.PartCount do
		...
		if surfaceAlignedCFrame then
			...
			Offset = CFrame.new(AnchorPoint.Position + worldOffset)
		...
		if not Raycast then
			return -- Skip this part instead of returning entirely
		end
