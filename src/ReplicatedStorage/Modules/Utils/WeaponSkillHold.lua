...
function WeaponSkillHold:OnInputBegan(player, character)
	...
	if StateManager.StateCheck(character, "Actions", self.skillName) then
...
function WeaponSkillHold:ApplyHoldEffect(character, isHolding)
	...
	if isHolding then
		-- Lock position using StateManager
		StateManager.TimedState(character, "Actions", "WeaponSkillHold", 999) -- Long duration, will be manually removed

		-- Lock movement speeds (SpeedSet0 = set speed to 0)
		StateManager.TimedState(character, "Speeds", "WeaponSkillHoldSpeedSet0", 999)
		...
		-- Remove position lock using StateManager
		StateManager.RemoveState(character, "Actions", "WeaponSkillHold")
		---- print("[WeaponSkillHold] Removed Actions state")

		-- Remove speed lock
		StateManager.RemoveState(character, "Speeds", "WeaponSkillHoldSpeedSet0")
...
function WeaponSkillHold:MonitorForInterruptions(player, character)
	...
	while heldData and heldData.isHolding do
		...
		-- Check for stuns using ECS StateManager
