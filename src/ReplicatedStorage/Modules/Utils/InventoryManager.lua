...
local function syncInventoryToClient(entity)
    ...
    print("[InventoryManager] syncInventoryToClient called for entity:", entity)

    -- Get player from entity
    if not world:has(entity, comps.Player) then
        warn("[InventoryManager] Entity", entity, "has no Player component!")
        return
    end

    local player = world:get(entity, comps.Player)
    if not player or not player.Parent then
        warn("[InventoryManager] Player is nil or has no Parent")
        return
    end
    ...
    if not world:has(entity, comps.Inventory) or not world:has(entity, comps.Hotbar) then
        warn("[InventoryManager] Entity missing Inventory or Hotbar component")
        return
    end
    ...
    for slot, item in pairs(inventory.items) do
        ...
        table.insert(itemArray, {
            ...
            rarity = item.rarity or "common" -- Include rarity
        })
    end
    ...
    print("[InventoryManager] 🚀 FIRING Bridges.Inventory to", player.Name, "with", #itemArray, "items")
    Bridges.Inventory:Fire(player, syncData)
    print("[InventoryManager] ✅ Bridges.Inventory fired successfully")
end
...
function InventoryManager.addItem(entity, itemName, itemType, quantity, singleuse, description, icon, rarity)
    if not world:has(entity, comps.Inventory) then
    ...
    newItems[emptySlot] = {
        ...
        stackable = stackable,
        slot = emptySlot,
        rarity = rarity or "common" -- Default to common if not specified
    }
...
-- Force sync inventory to client (used to ensure inventory is sent on first spawn)
function InventoryManager.forceSync(entity)
    if not isServer then return end
    syncInventoryToClient(entity)
end
