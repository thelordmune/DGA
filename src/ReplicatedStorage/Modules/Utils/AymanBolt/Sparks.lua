...
local LightningBolt = require(script.Parent.Bolts)
...
function LightningSparks.Create(LightningBoltInstance, Settings)
	Settings = Settings or {}

	-- Merge user settings with defaults
	local spark = MergeSettings(DefaultSettings, Settings)

	-- Set required reference (not part of defaults)
	spark.LightningBolt = LightningBoltInstance
...
function LightningSparks.Init(ActiveSparks, dt)
	for _, ThisSpark in ActiveSparks do
		if ThisSpark.Enabled == true and ThisSpark.SparksN < ThisSpark.MaxSparkCount then
			...
			for _ = 1, rng:NextInteger(1, ThisSpark.MaxSparkCount - ThisSpark.SparksN) do
				...
				if #available_slots ~= 0 then
					...
					local NewSpark = LightningBolt.Create(
						A1,
						A2,
						{
							PartCount = rng:NextInteger(ThisSpark.MinPartsPerSpark, ThisSpark.MaxPartsPerSpark),
						}
					)
					...
					local cH, _, cV = Part.Color:ToHSV()
		...
		for i, v in ThisSpark.SlotTable do
			if v.Parts[1].Parent ~= nil then
				...
				table.remove(ThisSpark.SlotTable, i) --Removes reference to prevent memory leak
			end
		end
