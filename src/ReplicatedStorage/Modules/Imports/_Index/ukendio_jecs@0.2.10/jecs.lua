...
local function ECS_GENERATION_INC(e: i53)
	if e > ECS_ENTITY_MASK then
		...
		if next_gen > ECS_GENERATION_MASK then return id end
...
local function ECS_PAIR(pred: i53, obj: i53): i53
	return ECS_COMBINE(ECS_ENTITY_T_LO(obj), ECS_ENTITY_T_LO(pred))
		+ FLAGS_ADD(--[[isPair]] true) :: i53
end
...
local function entity_index_try_get_any(entity_index: EntityIndex, entity: number): Record?
	...
	if not r or r.dense == 0 then return nil end
...
local function entity_index_try_get(entity_index: EntityIndex, entity: number): Record?
	...
	if r then
		...
		if r_dense > entity_index.alive_count then return nil end
		if entity_index.dense_array[r_dense] ~= entity then return nil end
	end
	return r
end
...
local function entity_index_get_alive(index: EntityIndex, e: i24): i53
	...
	if r then return index.dense_array[r.dense] end
	return 0
end
...
local function archetype_move(
	...
	for i, column in src_columns do
		...
		if tr then dst_columns[tr.column][dst_row] = column[src_row] end
	...
	if src_row ~= moved then src_entities[src_row] = e2 end
...
local function world_has(world: World, entity: number, ...: i53): boolean
	...
	for i = 1, select("#", ...) do
		if not records[select(i, ...)] then return false end
	end
...
local function world_target(world: World, entity: i53, relation: i24, index: number?): i24?
	...
	if not idr then return nil end

	local tr = idr.cache[archetype.id]
	if not tr then return nil end

	local count = tr.count
	if nth >= count then nth = nth + count + 1 end
...
local function id_record_ensure(world: World, id: number): IdRecord
	...
	if not idr then
		...
		local on_add, on_set, on_remove =
			world_get(world, relation, EcsOnAdd, EcsOnSet, EcsOnRemove)
...
local function archetype_ensure(world: World, types): Archetype
	if #types < 1 then return world.ROOT_ARCHETYPE end

	local ty = hash(types)
	local archetype = world.archetypeIndex[ty]
	if archetype then return archetype end
...
local function find_archetype_without(world: World, node: Archetype, id: i53): Archetype
	...
	if at == nil then return node end
...
local function init_edge_for_add(world, archetype, edge: GraphEdge, id, to)
	...
	if archetype ~= to then
		...
		if next_edge then next_edge.prev = edge end
	end
end
...
local function init_edge_for_remove(
	...
	if archetype ~= to then
		...
		if prev_edge then prev_edge.next = edge end
	end
end
...
local function archetype_traverse_add(world: World, id: i53, from: Archetype?): Archetype
	...
	local to = edge.to
	if not to then to = create_edge_for_add(world, from, edge, id) end
...
local function archetype_traverse_remove(world: World, id: i53, from: Archetype): Archetype
	...
	local to = edge.to
	if not to then to = create_edge_for_remove(world, from, edge, id) end
...
local function world_add(world: World, entity: i53, id: i53): ()
	...
	if from then
		...
		if #to.types > 0 then new_entity(entity, record, to) end
	end

	local idr = world.componentIndex[id]
	local on_add = idr.hooks.on_add

	if on_add then on_add(entity) end
end
...
local function world_set(world: World, entity: i53, id: i53, data: unknown): ()
	...
	if from == to then
		...
		if on_set then on_set(entity, data) end
	...
	local on_set = idr_hooks.on_set
	if on_set then invoke_hook(on_set, entity, data) end
end
...
local function world_remove(world: World, entity: i53, id: i53)
	...
	if from and not (from == to) then
		...
		if on_remove then on_remove(entity) end

		entity_move(entity_index, entity, record, to)
	end
end

local function archetype_fast_delete_last(
	columns: { Column },
	column_count: number,
	types: { i53 },
	entity: i53
	...
	for i, column in columns do
		if column ~= NULL_ARRAY then column[column_count] = nil end
	end
end
...
local function archetype_delete(world: World, archetype: Archetype, row: number, destruct: boolean?)
	...
	if row ~= last then
		...
		if record_to_move then record_to_move.row = row end
	end
	...
	for _, id in types do
		...
		if on_remove then on_remove(delete) end
	end
...
local function archetype_disconnect_edge(edge: GraphEdge)
	...
	if edge_next then edge_next.prev = edge_prev end
	if edge_prev then edge_prev.next = edge_next end
end
...
local function archetype_destroy(world: World, archetype: Archetype)
	if archetype == world.ROOT_ARCHETYPE then return end
	...
	for id in records do
		...
		idr.size -= 1
		records[id] = nil :: any
		if idr.size == 0 then component_index[id] = nil :: any end
	end
end
...
local function world_cleanup(world: World)
	...
	for _, archetype in archetypes do
		if #archetype.entities == 0 then archetype_destroy(world, archetype) end
	end
		...
		if idr_t then
			for archetype_id in idr_t.cache do
				...
				for _, id in idr_t_types do
					if not ECS_IS_PAIR(id) then continue end
...
local function query_iter_init(query): () -> (number, ...any)
	...
	if not archetype then return NOOP :: () -> (number, ...any) end
	local columns = archetype.columns
	local entities = archetype.entities
	local i = #entities
	local records = archetype.records
...
local function query_iter(query): () -> (number, ...any)
	...
	if not query_next then query_next = query_iter_init(query) end
	return query_next
end
...
local function query_without(query: { compatible_archetypes: { Archetype } }, ...)
	...
	for i = #compatible_archetypes, 1, -1 do
		...
		if shouldRemove then
			...
			if last ~= i then compatible_archetypes[i] = compatible_archetypes[last] end
			compatible_archetypes[last] = nil :: any
		end
	end

	if #compatible_archetypes == 0 then return EMPTY_QUERY end
...
local function world_query(world: World, ...)
	...
	for _, id in ids do
		...
		if not map then return EMPTY_QUERY end

		if idr == nil or map.size < idr.size then idr = map end
	end

	if not idr then return EMPTY_QUERY end

	for archetype_id in idr.cache do
		local compatibleArchetype = archetypes[archetype_id]
		if #compatibleArchetype.entities == 0 then continue end
	...
	if length == 0 then return EMPTY_QUERY end
...
if _G.__JECS_DEBUG then
	...
	local function get_name(world, id): string
		...
		if ECS_IS_PAIR(id) then
			name = `pair({get_name(world, ECS_ENTITY_T_HI(id))}, {get_name(
				world,
				ECS_ENTITY_T_LO(id)
			)})`
		else
			local _1 = world_get_one_inline(world, id, EcsName)
			if _1 then name = `${_1}` end
		end
		if name then
			return name
		else
	...
	World.add = function(world: World, entity: i53, id: i53, value: nil)
		if value ~= nil then
			...
			local _2 = get_name(world, id)
			throw(
				"You provided a value when none was expected. "
	...
	World.get = function(world: World, entity: i53, ...)
		...
		for i = 1, length do
			...
			if id_is_tag then
				...
				if not _1 then _1 = get_name(world, entity) end
				throw(
