...
type world = {
	...
	bulk_add: (self: world, entity: i53, ids: { i53 }, values: { any }) -> (),
	bulk_remove: (self: world, entity: i53, ids: { i53 }) -> (),
}
...
export type World = {
	...
	--- Bulk add multiple components in a single archetype transition (performance optimization)
	bulk_add: <T>(self: World, entity: Entity<T>, ids: { Id<any> }, values: { any }) -> (),
	--- Bulk remove multiple components in a single archetype transition (performance optimization)
	bulk_remove: <T>(self: World, entity: Entity<T>, ids: { Id<any> }) -> (),
}
...
local function world_new()
	...
	-- Bulk operations for performance (single archetype transition for multiple components)
	world.bulk_add = function(self: world, entity: i53, ids: { i53 }, values: { any })
		return ecs_bulk_insert(self, entity, ids, values)
	end
	world.bulk_remove = function(self: world, entity: i53, ids: { i53 })
		return ecs_bulk_remove(self, entity, ids)
	end
