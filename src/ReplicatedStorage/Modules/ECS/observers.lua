...
local function monitors_new<T...>(query: Query<T...>): Monitor<T...>
	...
	local function archetype_changed<a>(entity: jecs.Entity, src: jecs.Archetype, dst: jecs.Archetype)
		if archetypes[dst.id] then
			if not archetypes[src.id] then
				if callback_added and type(callback_added) == "function" then
					callback_added(entity)
				elseif callback_added then
					warn(`[observers] callback_added is not a function: {type(callback_added)}`)
				end
			end
		else
			if archetypes[src.id] then
				if callback_removed and type(callback_removed) == "function" then
					callback_removed(entity)
				elseif callback_removed then
					warn(`[observers] callback_removed is not a function: {type(callback_removed)}`)
				end
			end
		end
	end
	...
	local function monitor_removed(self, callback)
		if type(callback) ~= "function" then
			error(`[observers] monitor:removed() expects a function, got {type(callback)}`)
		end
