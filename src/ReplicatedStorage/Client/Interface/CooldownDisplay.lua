...
local function getSkillCooldownDuration(skillName: string): number
	...
	-- Try weapon skills
	local weaponSkillsModule = game:GetService("ServerStorage"):FindFirstChild("Stats")
	if weaponSkillsModule then
		local skillsData = weaponSkillsModule:FindFirstChild("_Skills")
		if skillsData then
			local weapon = player:GetAttribute("Weapon") or "Fist"
			local success, skills = pcall(function()
				return require(skillsData)
			end)

			if success and skills[weapon] and skills[weapon][skillName] then
				-- Weapon skills might not have explicit cooldown, use default
