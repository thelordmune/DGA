...
local TInfo = TweenInfo.new(.5, Enum.EasingStyle.Circular, Enum.EasingDirection.InOut, 0, true) -- PingPong for automatic back-and-forth
...
return function(scope, props: {})
	...
	scope:New "ScrollingFrame" {
		...
		Position = scope:Spring(
			scope:Computed(function(use)
				return if use(started) then UDim2.fromScale(.899, -.03) else UDim2.fromScale(1.2, -.03)
			end),
			5,
			.8
		),
		...
		[Children] = {
			...
			scope:New "Frame" {
				...
				BorderColor3 = Color3.fromRGB(0, 0, 0),
				BorderSizePixel = 0,
				Position = UDim2.fromScale(0.12, .016),
				Size = UDim2.new(.77,0,0,891),
	...
	scope:New "ImageLabel" {
		...
		Position = scope:Spring(
			scope:Computed(function(use)
				return if use(started) then UDim2.fromScale(.902, -.03) else UDim2.fromScale(1.2, -.03)
			end),
			5,
			.8
		),
