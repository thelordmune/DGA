        ...
        if WeaponFolder and not table.find(blacklist, WeaponName) then
            ...
            if not hasWeaponParts then
                ...
                for _, WepPart in pairs(weaponParts) do
                    ...
                    -- Connect Equip weld for equipped position
                    if PotentialPart:FindFirstChild("Equip") then
                        ...
                        -- Ensure Unequip weld exists but is disconnected (for later use)
...
NetworkModule.UnequipWeapon = function(Character: Model, WeaponName: string?, skipAnimation: boolean?)
    ...
    local function disconnectWelds()
        ...
        -- If weapon parts don't exist, spawn them with Unequip weld connected
        if not hasWeaponParts then
            local WeaponFolder = ServerStorage.Assets.Models.Weapons[WeaponName]
            if WeaponFolder then
                for _, WepPart in pairs(WeaponFolder:GetChildren()) do
                    local PotentialPart = WepPart:Clone()

                    -- Connect Unequip weld for unequipped position
                    if PotentialPart:FindFirstChild("Unequip") then
                        if PotentialPart.Unequip:GetAttribute("Part0") then
            ...
            -- Weapon parts exist, just disconnect Equip and connect Unequip
            for _, PotentialPart in pairs(Character:GetChildren()) do
                if PotentialPart:GetAttribute("Weapon") then
                    -- Disconnect Equip weld if it exists
                    if PotentialPart:FindFirstChild("Equip") then
                        if PotentialPart.Equip:GetAttribute("Part0") then
                    ...
                    -- Connect Unequip weld if it exists
                    if PotentialPart:FindFirstChild("Unequip") then
                        if PotentialPart.Unequip:GetAttribute("Part0") then
                            PotentialPart.Unequip.Part0 = Character[PotentialPart.Unequip:GetAttribute("Part0")]
                        else
    ...
    -- Weapons that should stay visible when unequipped (for special idle animations)
    local keepVisibleWhenUnequipped = {"Spear"}
    local shouldKeepVisible = table.find(keepVisibleWhenUnequipped, WeaponName)
