	...
	if cameraSubject then
		if cameraSubject:IsA("Humanoid") then
			...
			if bodyPartToFollow and bodyPartToFollow:IsA("BasePart") then
				...
				rawPosition = bodyPartToFollow.CFrame.p + bodyPartToFollow.CFrame:vectorToWorldSpace(heightOffset + cameraOffset)
			end

		elseif cameraSubject:IsA("VehicleSeat") then
			local offset = SEAT_OFFSET
			rawPosition = cameraSubject.CFrame.p + cameraSubject.CFrame:vectorToWorldSpace(offset)
		elseif cameraSubject:IsA("SkateboardPlatform") then
			rawPosition = cameraSubject.CFrame.p + SEAT_OFFSET
		elseif cameraSubject:IsA("BasePart") then
			rawPosition = cameraSubject.CFrame.p
		elseif cameraSubject:IsA("Model") then
			if cameraSubject.PrimaryPart then
				rawPosition = cameraSubject:GetPrimaryPartCFrame().p
			else
				rawPosition = cameraSubject:GetModelCFrame().p
			end
		end
	else
		return nil
