...
screengui.DisplayOrder = 100 -- High display order to ensure it's on top
...
-- Adjust camera (focus on HumanoidRootPart)
...
if humanoid then
	...
	animation.AnimationId = "rbxassetid://128209591266447"  -- Test animation
...
progressConnection = game:GetService("RunService").Heartbeat:Connect(function()
...
-- Note: ScreenGui doesn't have BackgroundTransparency, so we'll fade individual elements

-- Also fade out the viewport
